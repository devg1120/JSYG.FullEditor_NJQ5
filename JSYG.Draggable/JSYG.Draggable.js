...
export class Draggable extends StdConstruct {
    ...
    start(e) {
        ...
        function remove(e) {
		   ...
		   guides.trigger('success',that.node,e);
            ...
            doc[0].removeEventListener("mousemove", mousemoveFct);
            doc[0].removeEventListener("mouseup", remove);

            if (hasChanged) {
	        that.trigger('dragend',that.node,e);
		//that[0].dispatchEvent(new Event('dragend'));
	    ...
	    //that[0].dispatchEvent(new Event('end'));

            //const event2 = new CustomEvent("end", { detail: e });
            //that.node.dispatchEvent(event2);
        }
