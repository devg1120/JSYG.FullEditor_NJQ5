...
Path.prototype.createSeg = function (type) {
    ...
    switch (low) {
        ...
        case "c":
            ...
            break;
        case "q":
            method += "CurvetoQuadratic";
            break;
        case "a":
            method += "Arc";
            break;
        case "h":
            method += "LinetoHorizontal";
            break;
        case "v":
            method += "LinetoVertical";
            break;
        case "s":
            method += "CurvetoCubicSmooth";
            break;
        case "t":
            method += "CurvetoQuadraticSmooth";
            break;
        default:
            throw new Error(type + " is not a correct letter for drawing paths !");
    }
...
Path.prototype.cloneSeg = function (seg) {
    ...
    if (letter === "h") args.push(seg.x);
        ...
        switch (letter) {
            case "c":
                args.push(seg.x1, seg.y1, seg.x2, seg.y2);
                break;
            case "q":
                args.push(seg.x1, seg.y1);
                break;
            case "a":
                args.push(seg.r1, seg.r2, seg.angle, seg.largeArcFlag, seg.sweepFlag);
                break;
            case "s":
                args.push(seg.x2, seg.y2);
                break;
        }
    }
...
function rel2absSeg(jPath, ind) {
    ...
    if (letter === "h") args.push(ref.x + seg.x);
        ...
        switch (letter) {
            case "c":
                args.push(ref.x + seg.x1, ref.y + seg.y1, ref.x + seg.x2, ref.y + seg.y2);
                break;
            case "q":
                args.push(ref.x + seg.x1, ref.y + seg.y1);
                break;
            case "a":
                args.push(seg.r1, seg.r2, seg.angle, seg.largArcFlag, seg.sweepFlag);
                break;
            case "s":
                args.push(ref.x + seg.x2, ref.y + seg.y2);
                break;
        }
    }
...
Path.prototype.splitSeg = function (ind) {
    ...
    switch (letter) {
        ...
        default:
            throw new Error("You must normalize the jPath");
    }
...
Path.prototype.seg2Polyline = function (ind, nbsegs) {
    ...
    switch (letter) {
        ...
        default:
            throw new Error(
                "Vous devez normaliser le chemin pour applique la m̩thode seg2Polyline",
            );
    }
...
Path.prototype.toPolyline = function (nbsegs) {
    ...
    if (!this.isNormalized())
        throw new Error("Il faut normaliser le chemin pour la m̩thode toPolyLine");
    ...
    for (; i < N; i++) {
        ...
        if (letter === "H") {
            ...
            jPath.addSeg(
                ...
                seg.x,
                seg.y,
                (1 / 3) * currentPoint.x + (2 / 3) * seg.x1,
                currentPoint.y / 3 + (2 / 3) * seg.y1,
                (2 / 3) * seg.x1 + (1 / 3) * seg.x,
                (2 / 3) * seg.y1 + (1 / 3) * seg.y,
            );
        } else if (letter === "T") {
            //transform T to Q
...
function computeCenterAndAngles(startPoint, seg) {
    ...
    var coef =
            (seg.largeArcFlag === seg.sweepFlag ? -1 : 1) *
            Math.sqrt(
                Math.max(0, (r1c * r2c - r1c * yp1c - r2c * xp1c) / (r1c * yp1c + r2c * xp1c)),
            ),
        ...
        cosTheta =
            (xp1 - cpx) /
            seg.r1 /
            Math.sqrt(Math.pow((xp1 - cpx) / seg.r1, 2) + Math.pow((yp1 - cpy) / seg.r2, 2)),
        ...
        deltaTheta =
            ((u.x * v.y - u.y * v.x > 0 ? 1 : -1) *
                Math.acos(Math.max(-1, Math.min(1, cosDeltaTheta)))) %
            (Math.PI * 2);
...
function estimateError(seg, etaA, etaB, bezierDegree) {
    ...
    if (bezierDegree < 2) {
        ...
        yA =
            seg.cy +
            aCosEtaA * Math.sin(seg.angleRad) +
            Math.sin(seg.angleRad) * Math.cos(seg.angleRad);
        ...
        cos6 = Math.cos(6 * eta);
        coeffs =
            x < 0.25 ? coefs["degree" + bezierDegree].low : coefs["degree" + bezierDegree].high; // select the right coeficients set according to degree and b/a
        c0 =
            rationalFunction(x, coeffs[0][0]) +
            cos2 * rationalFunction(x, coeffs[0][1]) +
            cos4 * rationalFunction(x, coeffs[0][2]) +
            cos6 * rationalFunction(x, coeffs[0][3]);
        c1 =
            rationalFunction(x, coeffs[1][0]) +
            cos2 * rationalFunction(x, coeffs[1][1]) +
            cos4 * rationalFunction(x, coeffs[1][2]) +
            cos6 * rationalFunction(x, coeffs[1][3]);

        return (
            rationalFunction(x, coefs["degree" + bezierDegree].safety) *
            seg.r1 *
            Math.exp(c0 + c1 * dEta)
        );
    }
...
Path.prototype.arc2bez = function (ind, bezierDegree, defaultFlatness) {
    ...
    if (seg.pathSegTypeAsLetter !== "A") {
        throw new Error("You can only comput center and angles on 'A' segments");
    }
...
JSYG.prototype.isShapeInside = function (shape) {
    ...
    var jShape = new JSYG(shape)
        .clonePath({ normalize: true, transform: true })
        .css("visibility", "hidden")
        .appendTo(this.parent())
        .mtx2attrs()
        .toPolyline();
    var clone = this.clonePath({ normalize: true, transform: true })
        .css("visibility", "hidden")
        .appendTo(this.parent())
        .mtx2attrs()
        .toPolyline();
