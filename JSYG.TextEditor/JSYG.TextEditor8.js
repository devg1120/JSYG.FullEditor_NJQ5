...
export class TextEditor extends StdConstruct {
    constructor(arg, opt) {
        super(arg, opt);
...
class Selection extends StdConstruct {
...
class Cursor extends StdConstruct {
    constructor(textObject) {
        super(textObject);
        this.textEditor = textObject;
...
/*
    _keypress(e) {
        ...
        if (
            !/^[\w\W]$/.test(key) ||
            e.ctrlKey ||
            (this.textEditor.cursor.display === false && this.textEditor.display === false)
        )
            return;
...
inputField.addEventListener('compositionstart', (e) => {
  ...
  // ここでIME状態を記録（例: IMEモードオン）
});

// 変換中、テキスト更新時
inputField.addEventListener('compositionupdate', (e) => {
  console.log('Composition Update:', e.data); // e.dataで現在の変換候補テキスト
  // ここで変換中のテキストをプレビュー表示するなど
});

// 変換確定後
inputField.addEventListener('compositionend', (e) => {
  console.log('Composition End:', e.data); // e.dataで確定したテキスト
  // ここで確定後の処理（例: データベース保存）
});

// キー押下時（確定後のキー、またはIMEオフ時のキー）
inputField.addEventListener('keydown', (e) => {
  if (e.isComposing) { // 変換中なら何もしない（compositionupdateで処理済み）
    return;
  }
  console.log('Key Down:', e.key); // 確定したキー情報
});

*/
/*
 https://note.affi-sapo-sv.com/js-input-key.php
*/
