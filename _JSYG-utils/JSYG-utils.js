(function (root, factory) {
    if (typeof module == "object" && typeof module.exports == "object") {
        ...
        if (JSYG.Matrix && JSYG.Vect && JSYG.Point && JSYG.utf8encode)
            factory(JSYG, JSYG.Matrix, JSYG.Vect, JSYG.Point, JSYG);
            ...
            for (; i < N; i++) {
                ...
                if (defaultStyle[name] != value) {
                    ...
                    if (value.indexOf(href) != -1)
                        value = value.replace(href, "").replace(/"|'/g, "");
    ...
    JSYG.prototype.getDim = function (type) {
        ...
        if (isWindow(node)) {
            ...
            type === "screen" ||
            isWindow(type) ||
            (type instanceof $ && isWindow(type[0]))
        ) {
            jWin = new JSYG(window);
        ...
        this.each(function () {
            ...
            switch (tag) {
                ...
                case "use": //on peut répercuter x et y mais pas width ni height
                    ...
                    if (
                        ("width" in opt && dim.width != 0) ||
                        ("height" in opt && dim.height != 0)
                    ) {
                        mtx = new Matrix();
    ...
    JSYG.prototype.getShift = function (pivotX, pivotY) {
        ...
        if (JSYG.isNumeric(pivotX) && JSYG.isNumeric(pivotY))
        ...
        this.each(function () {
            ...
            switch (tag) {
                ...
                case "path":
                    if (!JSYG.Path)
                        throw new Error(
                            "Il faut inclure le module JSYG.Path pour pouvoir utiliser la méthode mtx2attrs sur les chemins",
                        );
    ...
    JSYG(function () {
        ...
        new JSYG("<rect>").attr({ id: id, x: 10, y: 10, width: 10, height: 10 }).appendTo(defs);
